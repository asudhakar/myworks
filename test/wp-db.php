...
function prepare_link_hash($this_email) {
	global $wpdb;

	$table_name = WPTC_CUST_USERS_TABLE;
	$query = $wpdb->prepare("SELECT * FROM `$table_name` WHERE email = %s ", $this_email);
	$query_result = $wpdb->get_results($query, ARRAY_A);

	$wptc_secret_hash_key = 'wp_tc_beta_cron_623';
	$to_be_hashed_text = serialize($query_result) . $wptc_secret_hash_key;

	$link_hash = urlencode(md5($to_be_hashed_text));
	return $link_hash;
}